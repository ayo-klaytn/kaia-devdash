...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  const repositories = await db.select().from(repository).limit(parseInt(limit)).offset(offset);
  ...
  for (const repo of repositories) {
    if (!repo.url) {
      ...
      await db.update(repository).set({ url: newUrl }).where(eq(repository.id, repo.id));
    }
    ...
    try {
      ...
      if (response.status === 200) {
        await db.update(repository).set({ status: "active" }).where(eq(repository.id, repo.id));
      } else {
        await db.update(repository).set({ status: "inactive" }).where(eq(repository.id, repo.id));
      }
    } catch (error) {
      console.error(error);
    }
