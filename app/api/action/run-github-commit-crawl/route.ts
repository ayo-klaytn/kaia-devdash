  ...
  try {
    ...
    if (runningProcess) {
      return NextResponse.json({ 
        message: "Repository commits update is already running",
        pid: runningProcess.pid
      });
    }

    // Start the process
    runningProcess = spawn('bun', ['scripts/getCommits.ts'], {
      detached: true,
      stdio: 'ignore'
    });
    ...
    return NextResponse.json({ 
      message: "Repository commits update started in background",
      pid: runningProcess.pid
    });
  } catch (error) {
    console.error("Error starting repository commits update:", error);
    return NextResponse.json({ error: "Failed to start repository commits update" }, { status: 500 });
  }
}
