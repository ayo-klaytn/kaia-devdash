...
async function fetchFromDune(): Promise<number> {
  ...
  const queryId = process.env.DUNE_MAX_TPS_QUERY_ID || "4674606";
  if (!apiKey) return 0;
  ...
  const candidates = ["max_tps", "maxTps", "tps", "value", "count"];
...
export async function GET() {
  try {
    ...
    const res = NextResponse.json({ maxTps: value });
    res.headers.set("Cache-Control", "public, s-maxage=600, stale-while-revalidate=1800");
    return res;
  } catch {
    return NextResponse.json({ maxTps: 0 }, { status: 200 });
  }
}
