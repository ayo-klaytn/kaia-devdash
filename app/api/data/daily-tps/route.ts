...
async function fetchFromDune(): Promise<number> {
  ...
  const queryId = process.env.DUNE_DAILY_TPS_QUERY_ID || '4225892';
  if (!apiKey) return 0;
  ...
  const candidates = [
    'daily_tps', 'dailyTps', 'tps', 'value', 'count',
  ];
...
export async function GET() {
  try {
    ...
    const res = NextResponse.json({ dailyTps: value });
    res.headers.set('Cache-Control', 'public, s-maxage=600, stale-while-revalidate=1800');
    return res;
  } catch {
    return NextResponse.json({ dailyTps: 0 }, { status: 200 });
  }
}
