...
// Keep period config in one place so pages and APIs stay aligned
export const GITHUB_PERIOD_CONFIG = [
...
export type GithubPeriodId = (typeof GITHUB_PERIOD_CONFIG)[number]["id"];
...
function resolvePeriod(id?: string) {
  const period = GITHUB_PERIOD_CONFIG.find((p) => p.id === id);
  // Default to kaia-2024 if nothing matches
  return period ?? GITHUB_PERIOD_CONFIG[GITHUB_PERIOD_CONFIG.length - 2];
}

function getPeriodBounds(periodId: GithubPeriodId): { start: string; end: string | null } {
  switch (periodId) {
    case "klaytn-2022":
      return {
...
/**
 * Core Github metrics computation used by both API routes and server components.
 * This keeps Stripe/PayPal-style logic in one place and avoids extra HTTP hops.
 */
export async function getGithubMetrics(periodParam?: string): Promise<GithubMetricsResponse> {
  const period = resolvePeriod(periodParam);

  // 1) Try DB-backed cache table (fastest)
  try {
    ...
    const cachedResult = (await Promise.race([queryPromise, timeoutPromise])) as
      | Array<{
          period_id: string;
          period_label: string;
          brand: string;
    ...
    if (cachedRows.length > 0) {
      ...
      const endDate = cached.end_date
        ? cached.end_date instanceof Date
          ? cached.end_date
          : new Date(cached.end_date)
        : null;

      const computedAt = cached.computed_at instanceof Date ? cached.computed_at : new Date(cached.computed_at);

      console.log(
        `[GitHub Metrics] ✅ Database cache HIT for period: ${period.id} (computed at ${computedAt.toISOString()}, query took ${dbCheckTime}ms)`,
      );
      ...
      return {
        ...
        availablePeriods: GITHUB_PERIOD_CONFIG.map(({ id, label, brand }) => ({
          id,
          label,
          brand,
        })),
    ...
    if (errorMsg.includes("timeout")) {
      console.error(
        `[GitHub Metrics] ❌ Database query timed out (3s) - DB connection may be slow. Error: ${errorMsg}`,
      );
    } else if (
      errorMsg.includes("does not exist") ||
      errorMsg.includes("relation") ||
      errorMsg.includes("github_metrics_cache")
    ) {
      console.error(
        `[GitHub Metrics] ❌ Table github_metrics_cache does not exist. Run: pnpm tsx scripts/add-github-metrics-cache-table-standalone.ts`,
      );
    } else {
  ...
  // 2) In-memory cache as a second line of defense
  ...
  console.log(
    `[GitHub Metrics] ❌ Cache MISS for period: ${period.id} - computing on-the-fly (this may take 60s)...`,
  );
  console.log(`[GitHub Metrics] 💡 Tip: Run 'pnpm tsx scripts/compute-github-metrics.ts' to pre-compute metrics`);

  // 3) Full computation as a slow fallback
  ...
  const repoRows = Array.isArray(repoRowsResult) ? repoRowsResult : repoRowsResult.rows ?? [];
  ...
  console.log(
    `[GitHub Metrics] ✅ Data generated for period: ${period.id}, caching for ${CACHE_TTL.GITHUB_METRICS} hours`,
  );
